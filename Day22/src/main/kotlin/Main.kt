...
private fun solvePart2() {
    ...
    // consider intersects between steps only which creates cuboids of on points

    val steps = input.toRebootSteps()
    val initialGrid = steps.toGrid()
    val answer = steps
        .runSteps(initialGrid)
        .count { it.isOn }
...
private fun List<RebootStep>.toGrid(): List<Point> {
    val minX = this.minOf { it.xRange.first }
    val maxX = this.maxOf { it.xRange.last }

    val minY = this.minOf { it.yRange.first }
    val maxY = this.maxOf { it.yRange.last }

    val minZ = this.minOf { it.zRange.first }
    val maxZ = this.maxOf { it.zRange.last }

    return createGrid(minX..maxX, minY..maxY, minZ..maxZ)
}
